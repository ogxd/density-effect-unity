Shader "Ogxd/Blur Effect"
{
	...
	float4 frag13Blur (output_13tap IN) : SV_Target
	{
	#if GAMMA_CORRECTION
		float3 sum = GammaToLinearSpace(tex2D(_MainTex, IN.texcoord).xyz) * 0.19648255;
		...
		//sum = LinearToGammaSpace(sum);
		//sum = hsv_to_rgb(float3(sum.r, 1, 1));
		sum = saturate(floor(sum * 20.0) / 20.0);
		return fixed4(sum, 1.0);
	#else
